...
// Backend: Melhor Envio produção + Mercado Pago, com preços validados no servidor.
...
function validateCart(raw) {
  ...
  const cart = raw.map((item) => {
    ...
    if (product.unavailable) throw new Error(`${product.name} está indisponível no momento.`);
...
function storeRequired(env) { if(!env.FPASSOS_STORE) throw new Error('Armazenamento FPASSOS_STORE ainda não foi configurado no Worker.'); return env.FPASSOS_STORE; }
...
async function readTokens(env) { try { return JSON.parse(await storeRequired(env).get('melhorenvio:tokens')||'null'); } catch { return null; } }
async function saveTokens(env, t) { const now=Date.now(); t.expires_at=now+(Number(t.expires_in||2592000)*1000); await storeRequired(env).put('melhorenvio:tokens',JSON.stringify(t)); return t; }
async function refreshMeToken(env, tokens) {
  ...
  const r=await fetch(`${meBase(env)}/oauth/token`,{method:'POST',headers:{Accept:'application/json','Content-Type':'application/x-www-form-urlencoded','User-Agent':meUserAgent(env)},body}); const data=await r.json();
  if(!r.ok) throw new Error(data.message||data.error_description||'Falha ao renovar Melhor Envio.'); return saveTokens(env,data);
}
async function getMeToken(env, forceRefresh=false) {
  let tokens=await readTokens(env); if(tokens?.access_token) { if(forceRefresh || (tokens.expires_at && tokens.expires_at-Date.now()<21600000)) tokens=await refreshMeToken(env,tokens); return tokens.access_token; }
  if(env.MELHOR_ENVIO_ACCESS_TOKEN) return env.MELHOR_ENVIO_ACCESS_TOKEN;
  const e=new Error('Melhor Envio ainda não conectado.'); e.code='MELHOR_ENVIO_NOT_CONNECTED'; throw e;
}
async function meFetch(env, path, init) {
  let token=await getMeToken(env); let r=await fetch(`${meBase(env)}${path}`,{...init,headers:{...meHeaders(token,env),...(init?.headers||{})}});
  if(r.status===401 && (await readTokens(env))?.refresh_token) { token=await getMeToken(env,true); r=await fetch(`${meBase(env)}${path}`,{...init,headers:{...meHeaders(token,env),...(init?.headers||{})}}); }
  return r;
}
async function shippingQuote(request, env) {
  try { const body=await request.json(), zip=digits(body.postal_code); if(zip.length!==8) return json({error:'CEP inválido.'},422); const {cart,subtotal}=validateCart(body.cart);
    const origin=digits(env.ORIGIN_ZIP||'04236290'); if(origin.length!==8) return json({error:'CEP de origem não configurado.'},500);
    const products=cart.map(x=>{const p=PRODUCTS[x.id];return {id:String(x.id),width:p.width,height:p.height,length:p.length,weight:p.weight,insurance_value:round2(p.price),quantity:x.quantity}});
    const r=await meFetch(env,'/api/v2/me/shipment/calculate',{method:'POST',body:JSON.stringify({from:{postal_code:origin},to:{postal_code:zip},products,options:{receipt:false,own_hand:false}})});
    const data=await r.json(); if(!r.ok) return json({error:data.message||data.error||'Melhor Envio recusou a cotação.',details:data},r.status);
    const handling=Number(env.SHIPPING_HANDLING_FEE||0), extraDays=Math.max(0,Number(env.SHIPPING_EXTRA_DAYS||0));
    const services=(Array.isArray(data)?data:[]).filter(s=>!s.error && (s.custom_price??s.price)!=null).map(s=>({id:String(s.id),name:s.name||'Entrega',company:s.company?.name||'',price:round2(Number(s.custom_price??s.price)+handling),delivery_time:Number((s.custom_delivery_time??s.delivery_time)||0)+extraDays})).sort((a,b)=>a.price-b.price);
    if(!services.length) return json({error:'Nenhuma transportadora retornou cotação para este carrinho/CEP.'},422);
    const quoteId=crypto.randomUUID(); await storeRequired(env).put(`quote:${quoteId}`,JSON.stringify({zip,cart,subtotal,services,created_at:Date.now()}),{expirationTtl:1800});
    return json({quote_id:quoteId,subtotal,services});
  } catch(e) { return json({error:e.message||'Erro no frete.',code:e.code||undefined},e.code==='MELHOR_ENVIO_NOT_CONNECTED'?503:400); }
}
async function meConnect(request, env) {
  ...
  const url=new URL(request.url), callback=`${url.origin}/api/melhor-envio/callback`, state=crypto.randomUUID(); await storeRequired(env).put(`oauth:${state}`,'1',{expirationTtl:600});
  const auth=new URL(`${meBase(env)}/oauth/authorize`); auth.searchParams.set('client_id',String(env.MELHOR_ENVIO_CLIENT_ID)); auth.searchParams.set('redirect_uri',callback); auth.searchParams.set('response_type','code'); auth.searchParams.set('state',state); auth.searchParams.set('scope','shipping-calculate'); return Response.redirect(auth.toString(),302);
}
async function meCallback(request, env) {
  try { const url=new URL(request.url),code=url.searchParams.get('code'),state=url.searchParams.get('state'); if(!code||!state||!(await storeRequired(env).get(`oauth:${state}`))) return new Response('Autorização inválida ou expirada.',{status:400}); await storeRequired(env).delete(`oauth:${state}`);
    const callback=`${url.origin}/api/melhor-envio/callback`; const body=new URLSearchParams({grant_type:'authorization_code',client_id:String(env.MELHOR_ENVIO_CLIENT_ID),client_secret:env.MELHOR_ENVIO_CLIENT_SECRET,redirect_uri:callback,code});
    const r=await fetch(`${meBase(env)}/oauth/token`,{method:'POST',headers:{Accept:'application/json','Content-Type':'application/x-www-form-urlencoded','User-Agent':meUserAgent(env)},body}); const data=await r.json(); if(!r.ok) return new Response(`Falha na autorização: ${data.message||data.error_description||r.status}`,{status:400}); await saveTokens(env,data);
    return new Response('<!doctype html><meta charset="utf-8"><title>FPassos</title><style>body{font-family:Arial;background:#080a0d;color:#fff;display:grid;place-items:center;height:100vh;margin:0}div{max-width:600px;text-align:center}h1{color:#ffc400}</style><div><h1>Melhor Envio conectado ✓</h1><p>A FPassos já pode calcular frete real. Você pode fechar esta página e voltar ao site.</p></div>',{headers:{'content-type':'text/html; charset=utf-8'}});
  } catch(e) { return new Response('Erro: '+e.message,{status:500}); }
}
async function meStatus(env) { const t=await readTokens(env); return json({connected:!!(t?.access_token||env.MELHOR_ENVIO_ACCESS_TOKEN),oauth_ready:!!(env.MELHOR_ENVIO_CLIENT_ID&&env.MELHOR_ENVIO_CLIENT_SECRET),origin_zip:digits(env.ORIGIN_ZIP||'04236290')}); }
...
async function processPayment(request, env) {
  try { if(!env.MERCADO_PAGO_ACCESS_TOKEN) return json({error:'MERCADO_PAGO_ACCESS_TOKEN ainda não foi cadastrado como Secret.'},503); const body=await request.json(); const quote=JSON.parse(await storeRequired(env).get(`quote:${body.quote_id}`)||'null'); if(!quote) return json({error:'Cotação de frete expirou. Calcule o CEP novamente.'},409);
    const {cart,subtotal}=validateCart(body.cart); if(JSON.stringify(cart)!==JSON.stringify(quote.cart)) return json({error:'O carrinho mudou após a cotação. Calcule o frete novamente.'},409); const service=quote.services.find(x=>String(x.id)===String(body.service_id)); if(!service) return json({error:'Opção de frete inválida.'},409);
    const customer=body.customer||{}, fd=body.form_data||{}, total=round2(subtotal+Number(service.price)); const orderRef='FP-'+Date.now().toString(36).toUpperCase()+'-'+crypto.randomUUID().slice(0,6).toUpperCase(); const url=new URL(request.url);
    const mp={transaction_amount:total,description:`FPassos Suplementos - ${cart.reduce((n,x)=>n+x.quantity,0)} item(ns)`,payment_method_id:fd.payment_method_id,payer:cleanPayer(fd,customer),external_reference:orderRef,notification_url:`${url.origin}/api/mercadopago/webhook?source_news=webhooks`,metadata:{quote_id:body.quote_id,shipping_service_id:String(service.id),shipping_service:service.name,shipping_price:Number(service.price)}};
    if(fd.token)mp.token=fd.token;if(fd.installments)mp.installments=Number(fd.installments);if(fd.issuer_id)mp.issuer_id=Number(fd.issuer_id);
    if(!mp.payment_method_id) return json({error:'Meio de pagamento não informado pelo Mercado Pago.'},422);
    const idem=crypto.randomUUID(); const r=await fetch('https://api.mercadopago.com/v1/payments',{method:'POST',headers:{Accept:'application/json','Content-Type':'application/json',Authorization:`Bearer ${env.MERCADO_PAGO_ACCESS_TOKEN}`,'X-Idempotency-Key':idem},body:JSON.stringify(mp)}); const data=await r.json(); if(!r.ok)return json({error:data.message||data.error||'Mercado Pago recusou a solicitação.',cause:data.cause||data},r.status);
    const order={order_reference:orderRef,payment_id:String(data.id),status:data.status,status_detail:data.status_detail,total,subtotal,shipping:service,cart:publicCart(cart),customer,created_at:new Date().toISOString()}; await storeRequired(env).put(`order:${data.id}`,JSON.stringify(order),{expirationTtl:60*60*24*180}); await storeRequired(env).put(`orderref:${orderRef}`,String(data.id),{expirationTtl:60*60*24*180});
    const tx=data.point_of_interaction?.transaction_data||{}; return json({id:String(data.id),order_reference:orderRef,status:data.status,status_detail:data.status_detail,payment_method_id:data.payment_method_id,pix:{qr_code:tx.qr_code||null,qr_code_base64:tx.qr_code_base64||null},ticket_url:tx.ticket_url||data.transaction_details?.external_resource_url||null});
  } catch(e) { return json({error:e.message||'Erro ao processar pagamento.'},400); }
}
...
async function health(env) {
  ...
  return json({
    ok: true,
    version: '2026-08-20-api-v3',
    store: Boolean(env.FPASSOS_STORE),
    mercado_pago: Boolean(env.MERCADO_PAGO_ACCESS_TOKEN),
    melhor_envio: melhorEnvio,
    origin_zip: digits(env.ORIGIN_ZIP || '04236290')
  });
}
